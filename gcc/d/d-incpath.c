/* d-incpath.c -- D frontend for GCC.
...
/* Routines to add import and file paths for the frontend.  */
...
/* Add PATHS to the global import/file lookup PATH.  */

static void
add_paths (Strings *paths, Strings *path)
{
  if (paths)
    {
      for (size_t i = 0; i < paths->dim; i++)
	{
	  ...
	  char *target_dir = make_absolute (CONST_CAST (char *, p));
...
void
d_frontend_add_paths (const char *iprefix, const char *imultilib, bool stdinc)
{
  if (stdinc)
    {
  ...
  if (global.params.imppath)
      ...
      for (size_t i = 0; i < global.params.imppath->dim; i++)
	{
	  ...
	  if (path)
	    add_paths (FileName::splitPath (path), global.path);
	}
    }
  ...
  if (global.params.fileImppath)
      ...
      for (size_t i = 0; i < global.params.fileImppath->dim; i++)
	{
	  ...
	  if (path)
	    add_paths (FileName::splitPath (path), global.filePath);
	}
    }
}
